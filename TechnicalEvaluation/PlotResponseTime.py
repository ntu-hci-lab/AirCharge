...
# targetAngle = 90 
targetAngles = [15,30,45,60,75,90]
# path = os.path.realpath('./Angle' + str(targetAngle))
pathAngle0 = os.path.realpath('./Angle0')
pathAngleAll = [os.path.realpath('./Angle' + str(angle)) for angle in targetAngles]
...
for index, files in enumerate(allFilesAngleAll):
    for file in files:
        ...
        x = sum(df_tmp['Time'])/len(df_tmp['Time'])
        axis.plot(df_tmp['Time'], label = str(targetAngles[index]) + ' degree', color = [np.random.random(),np.random.random(),np.random.random()])
